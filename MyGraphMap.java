...
public class MyGraphMap
{
    ...
    public void addVertex(String label, String value){
        if (label == null) {
            ...
            nodeMap.put(label, new Vertex(label, value));
        }
    }
    

    /**
     *  Sets the value of the vertex that label hashes to
     */
    public void setVertex(String label, String value) {
        nodeMap.get(label).setValue(value);
    }
    
    public Set<String> getAdjacency(String label) {
        if (label == null) {
            throw new NullPointerException();
        }
    
        Vertex mappedVertex = nodeMap.get(label);
        if (mappedVertex == null) {
            throw new NoSuchElementException();
        }
    
        Set<String> adjacentLabels = new HashSet<>();
        for (Vertex v : mappedVertex.getAdjVerticies()) {
            adjacentLabels.add(v.getLabel());
        }
    ...
    /**
     * Checks if there are any duplicate values in the adj verticies of the specified Vertex.
     */
    public boolean noAdjDups(String startLabel) {
        if (startLabel == null) {
            throw new NullPointerException(); 
        } else {
            Vertex startVertex = nodeMap.get(startLabel);
            if (startVertex == null) {
                throw new NoSuchElementException();
            } else {
                String startValue = startVertex.getValue();
                ArrayList<Vertex> adjVerticies = startVertex.getAdjVerticies();
                for (int i = 0 ; i < adjVerticies.size(); i++) {
                    String adjVertexValue = adjVerticies.get(i).getValue();
                    if (adjVertexValue.equals(startValue)) {
                        return true;
                    }
                }
                return false; 
